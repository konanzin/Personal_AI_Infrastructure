...
Task({ subagent_type: "general-purpose", description: "[topic] angle 1", prompt: "Search for: [angle 1]. Tag each finding with confidence: [HIGH], [MED], or [LOW]. Return findings." })
Task({ subagent_type: "general-purpose", description: "[topic] angle 2", prompt: "Search for: [angle 2]. Tag each finding with confidence: [HIGH], [MED], or [LOW]. Return findings." })

// Breadth - 3 threads (multi-perspective, cross-domain)
Task({ subagent_type: "general-purpose", description: "[topic] angle 3", prompt: "Search for: [angle 3]. Tag each finding with confidence: [HIGH], [MED], or [LOW]. Return findings." })
...
Task({ subagent_type: "general-purpose", description: "verify [topic] claims", prompt: "Independently verify the most commonly cited facts, statistics, and claims about [topic]. For each claim you find, check if it's supported by primary sources. Tag each as [HIGH] (confirmed), [MED] (plausible), or [LOW] (unconfirmed). Focus on quantitative claims and dates — these are most likely to be wrong." })
Task({ subagent_type: "general-purpose", description: "find contradictions about [topic]", prompt: "Search for contradictory evidence, debunked claims, and common misconceptions about [topic]. What do people get wrong? What's the contrarian view with evidence? Tag each finding with confidence: [HIGH], [MED], or [LOW]." })
```
...
⚠️ CONFLICT on [topic]:
  Explorer (angle agent): [claim] — [source]
  Verifier (independent agent): [contradicting claim] — [source]
  Status: Unresolved
...
- Explorer Angles: depth(2), breadth(3), contrarian(2)
- Verifier Agents: independent(2)
