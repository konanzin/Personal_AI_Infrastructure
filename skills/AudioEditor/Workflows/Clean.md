...
(curl -s --max-time 2 -X POST http://localhost:31337/notify \
  ...
  -d '{"message": "Running the Clean workflow in the AudioEditor skill to clean audio", "language": "en-US"}' \
  > /dev/null 2>&1 || true) &
```
...
```bash
bun ~/.config/opencode/skills/AudioEditor/Tools/Pipeline.ts \
  "<audio-file-path>" \
...
```bash
# Transcription only
bun ~/.config/opencode/skills/AudioEditor/Tools/Transcribe.ts <file>

# Analysis only (requires transcript)
bun ~/.config/opencode/skills/AudioEditor/Tools/Analyze.ts <transcript.json>

# Edit only (requires audio + edits)
bun ~/.config/opencode/skills/AudioEditor/Tools/Edit.ts <file> <edits.json>

# Polish only (requires CLEANVOICE_API_KEY)
bun ~/.config/opencode/skills/AudioEditor/Tools/Polish.ts <file>
```
