...
`~/.config/opencode/PAI/USER/SKILLCUSTOMIZATIONS/Fabric/`
...
1. **Send voice notification**:
   ...
   curl -s -X POST http://localhost:31337/notify \
     ...
     -d '{"message": "Running the WORKFLOWNAME workflow in the Fabric skill to ACTION", "language": "en-US"}' \
     > /dev/null 2>&1 &
   ```
...
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","skill":"Fabric","workflow":"WORKFLOW_USED","input":"8_WORD_SUMMARY","status":"ok|error","duration_s":SECONDS}' >> ~/.config/opencode/PAI/MEMORY/SKILLS/execution.jsonl
```
