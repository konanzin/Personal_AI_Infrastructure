...
curl -s -X POST http://localhost:31337/notify \
  ...
  -d '{"message": "MESSAGE", "language": "en-US", "voice_id": "fTtv3eikoepIosk8dTZ5", "voice_enabled": true}'
```
...
1. `mkdir -p ~/.config/opencode/PAI/PAI/MEMORY/WORK/{slug}/` (slug: `YYYYMMDD-HHMMSS_kebab-task-description`)
...
```bash
rg -i "TOPIC" ~/.config/opencode/PAI/PAI/MEMORY/KNOWLEDGE/ --type md -l
```
...
```bash
rg -l "KEYWORD1|KEYWORD2|KEYWORD3" ~/.config/opencode/PAI/projects/${HARNESS_USER_DIR}/memory/feedback_*.md
```
...
`Monitor({ description: "Agent watchdog", persistent: true, timeout_ms: 3600000, command: "bun $HOME/.config/opencode/PAI/PAI/TOOLS/AgentWatchdog.ts" })`
...
Every `[ ]`→`[x]` ISC transition you write to the ISA fires `CheckpointPerISC.hook.ts` (PostToolUse Edit/Write/MultiEdit). For each repo in `~/.config/opencode/PAI/checkpoint-repos.txt` that has uncommitted changes, the hook auto-commits with subject `ISC-{N} ({slug}): {description}` and flags `--no-verify --no-gpg-sign` (so husky/GPG never hang the session). Idempotent via sidecar `MEMORY/WORK/{slug}/.checkpoint-state.json` — no double-commits, no commits when nothing changed.

You do not need to do anything to use this — write `[x]` honestly per Inline Verification, and the checkpoint trail forms itself. The trail enables clean rollback to any prior ISC state via `bun ~/.config/opencode/PAI/PAI/TOOLS/Checkpoint.ts {list|show|rollback} <slug> [<isc-id>]`. Rollback is preview-only — it prints the suggested `git reset --hard <sha>` per repo and exits. {{PRINCIPAL_NAME}} runs the reset himself if he wants the rollback (per `feedback_no_worktree_isolation_without_consent`).
...
bun ~/.config/opencode/PAI/PAI/TOOLS/Inference.ts --mode advisor --auto-state \
...
bun ~/.config/opencode/PAI/PAI/TOOLS/Inference.ts --mode advisor \
  "TASK: ..." "STATE: ..." "QUESTION: ..."
```

Or programmatically:
```typescript
import { advisor } from "~/.config/opencode/PAI/PAI/TOOLS/Inference";
...
```bash
rg -l "TOPIC|KEYWORD" ~/.config/opencode/PAI/PAI/MEMORY/KNOWLEDGE/ --type md
```

**Forbidden destinations — writes here are CRITICAL FAILURE:**
- `~/.config/opencode/PAI/projects/${HARNESS_USER_DIR}/memory/` — harness auto-memory dir; blocked by `settings.json` permissions deny + `autoMemoryEnabled: false`. The router never routes here.
- Anywhere outside `~/.config/opencode/PAI/` for PAI doctrine — public repos are scrubbed via `<your-release-skill>` release workflow only.
...
1. Read ISA from `~/.config/opencode/PAI/PAI/MEMORY/WORK/` — full state
