...
**2. Delegation-capability floor (SOFT, v6.1.0).** Delegation capabilities (Anvil, Cato, Agent Teams, Custom Agents, Background Agents, Worktree Isolation, Research, etc.) remain show-your-math relaxable — sometimes the work is genuinely single-author and delegation adds noise.
...
- **Anvil** (configured engine, `USER/Config/anvil.json`) — never auto-included by tier; delegate when a second engine's diversity or context breadth is worth the cost. Always invoke when {{PRINCIPAL_NAME}} names "Anvil". Structured `unavailable` when no engine is configured.
