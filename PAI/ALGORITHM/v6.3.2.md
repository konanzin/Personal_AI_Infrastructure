...
The executor honors the stored classification when present. If `MODE` is MINIMAL or NATIVE, use the corresponding format from CLAUDE.md and stop. If `MODE` is ALGORITHM, enter the Algorithm at the named `TIER`. If classifier state is missing or stale, use the injected mode rules and fail safe to ALGORITHM E3 when unsure.
...
2. Otherwise honor stored classifier output when available.
...
**Fail-safe.** Any classifier error path defaults to ALGORITHM E3 with `SOURCE: fail-safe` in REASON. Conservative-by-default: under-escalation is the failure mode this system was built to prevent.
...
**Set effort level (v6.3.1 — OpenCode classifier-driven):**
1. Check for explicit E-level override (`/e1`-`/e5` or `E1`-`E5`, case-insensitive). If found: use that tier, set `effort_source: explicit`.
2. **Read `MODE` and `TIER` from the injected OpenCode runtime context**. The source of truth is the classifier object persisted in `MEMORY/STATE/current-work-<session>.json` by `pai-hooks.js`. If `MODE: ALGORITHM`, use `TIER` verbatim, set `effort_source: classifier`.
