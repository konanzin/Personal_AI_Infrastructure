...
**Mode and tier are decided by the OpenCode PAI plugin before substantive execution.** The runtime path is `opencode/plugins/pai-hooks.js` `chat.message` plus `opencode/plugins/lib/mode-classifier.lib.js`. The default classifier is deterministic and local; an LLM classifier is optional via environment configuration. The plugin persists the normalized result to `MEMORY/STATE/current-work-<session>.json` and injects it into the system context on the next transform.
...
**Coverage.** The classifier fires through the OpenCode `chat.message` hook for top-level chat messages. Subagent prompts inherit whatever the primary picked. LLM classification, when enabled, is optional and must fall back to the local classifier on timeout or provider failure.
