...
**OpenCode port status:** `syncISAToWorkRegistry()` already detects task ISAs under `MEMORY/WORK/` and standalone `ISA.md` files. `CheckpointPerISC` and ISA skill `Tools/*.ts` are not implemented in the port yet. Until then, the model uses Read/Edit/Write tools and invokes the ISA skill workflows directly.
...
OpenCode port status: automatic `CheckpointPerISC` commits are not implemented yet. Do not claim an ISC produced a git checkpoint unless you create and verify that checkpoint explicitly. Until the OpenCode checkpoint path exists, use normal git status/diff/commit commands when the user asks for durability, and record the gap in `## Decisions` for work that expected automatic checkpoints.
