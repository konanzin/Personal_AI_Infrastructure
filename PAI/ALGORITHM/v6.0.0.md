...
2. For task ISAs: `mkdir -p ~/.config/opencode/PAI/PAI/MEMORY/WORK/{slug}/` (slug: `YYYYMMDD-HHMMSS_kebab-task-description`)
...
Every `[ ]`→`[x]` ISC transition fires `CheckpointPerISC.hook.ts`. For each repo in `~/.config/opencode/PAI/checkpoint-repos.txt` with uncommitted changes, the hook auto-commits. Idempotent via sidecar `MEMORY/WORK/{slug}/.checkpoint-state.json`.
