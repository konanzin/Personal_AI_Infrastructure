...
- **Anvil** (configured long-context engine, `USER/Config/anvil.json`) — invoke at E3/E4/E5 when whole-project context materially affects correctness. Always invoke when {{PRINCIPAL_NAME}} names "Anvil". Structured `unavailable` when no engine is configured.
