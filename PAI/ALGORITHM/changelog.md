...
- **v6.3.2** (2026-06-14, OpenCode follow-up 2026-06-15) — **OpenCode core runtime release.** Adds real long-memory tools to the OpenCode port: `PAI/TOOLS/MemoryRetriever.ts` for BM25-lite lexical retrieval over `MEMORY/KNOWLEDGE/`, `PAI/TOOLS/KnowledgeGraph.ts` for stats/find/related/traverse over tags, wikilinks, and typed `related:` links, `PAI/TOOLS/SessionHarvester.ts` for dry-run-first transcript mining, `PAI/TOOLS/KnowledgeHarvester.ts` for status/validate/index/contradictions/conservative harvest, and `PAI/MEMORY/README.md` as the runtime memory manifest. Adds OpenCode-native `CheckpointPerISC` behavior in the plugin with allowlist-only commits, idempotent sidecar state, and `PAI/TOOLS/Checkpoint.ts` rollback preview. Active doctrine uses installed tools when available and falls back to `rg`/manual reads when stale installs are missing them. `LearningPatternSynthesis.ts` and Pulse memory consolidation remain deferred. Migration: `echo v6.3.2 > LATEST`. Rollback: `echo v6.3.1 > LATEST`.
