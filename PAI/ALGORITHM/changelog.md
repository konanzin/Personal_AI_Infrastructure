...
- **v6.3.6** (2026-07-08) — **Roster round two lands (drift register W2.14).** The four vendor-named researchers (Claude/Gemini/Grok/Perplexity) retire: the model-agnostic migration had already reduced them to byte-identical websearch agents on the session model — vendor identities with no vendor behind them. Research fans out general-purpose agents by ANGLE (depth, breadth, contrarian, current-state) with CodexResearcher as independent verifier. Anvil retires (zero invocations, no engine configured on any machine); the Arthur persona retires (deterministic Arthur.ts kept). Cato goes engine-agnostic (`USER/Config/cato.json`, codex/gpt-5.4 defaults). Delegation floor list drops Anvil. Basis: blind O4-research probe 3-1-0 pro-lean2, zero fabrication, 500s vs 912s — the baseline's vendor fan-out hung a task waiting on one of its own identical arms until the 600s cap. **Roster rule (canonical from this release):** an agent file earns its place only by carrying (a) a harness-enforced permission boundary or (b) external-engine wiring; personas and specialties belong in the spawn prompt. Migration: `echo v6.3.6 > LATEST`. Rollback: `echo v6.3.5 > LATEST` (and revert the W2.14 commit).
- **v6.3.5** (2026-07-08) — **Mandatory second-model producer retired (drift register W2.13, lean roster experiment).** The Forge agent, its `ForgeProgress.ts` wrapper, and BOTH E3-E5 producer auto-includes (Forge and Anvil) are gone: no tier forces code through a second engine. Anvil survives as the sole delegate producer, invoked by explicit name-match (always wins, any tier) or the model's own judgment, with structured `unavailable` when unconfigured. The delegation floor list and the EPHEMERAL FEATURE GATE example drop their Forge references. Basis: blind O4 lean-vs-bitter on the VPS rig (12 tasks, opus judges) — 9-3-0 pro-lean with zero fabrication, and the decisive codex-authenticated re-run where Forge-as-producer returned `unavailable-for-write` under the rig's own bwrap sandbox and cost 471s vs 187s for a weaker artifact (2026-07-08). Cross-vendor rationale was already dead on an OpenAI-primary harness (session gpt-5.5 → GPT-5.4 same family). Six persona agents (Engineer, Architect, Designer, Artist, Algorithm, Silas) retired in the same experiment; `/pai` now routes to the primary agent. Migration: `echo v6.3.5 > LATEST`. Rollback: `echo v6.3.4 > LATEST` (and revert the W2.13 commits to restore the roster).
- **v6.3.4** (2026-07-06) — **Thinking floor goes SOFT (drift register W2.8).** The tier thinking floor (E2 ≥2 … E5 ≥8) becomes show-your-math relaxable, symmetric with the delegation floor: reasoned under-floor in `## Decisions` is judgment, SILENT under-floor stays a violation. Basis: the HARD floor's premise (executor under-invests silently) was refuted by the 2026-07-06 escalation baseline — self-selection 47/47 on mode and tier with no classifier crutch (W2.2). **Deliberately NOT relaxed:** the v6.3.0 closed enumeration and phantom rule — their field evidence (executor wallpapered floors with invented labels; pointers leak vocabulary under pressure) stands and the W2.2 baseline measured tier selection, not capability honesty; retire them only via a W3.1 probe (model bump + wallpapering probe green). **Also in this release (W3.2):** the reflection echo's `implied_sentiment`/`satisfaction_prediction` `[1-10]` template slots — observed live being filled with a fixed 8 (prompt-fabricated precision, the W1.8 class one layer up) — are replaced by a single JUDGED `satisfaction` field with an explicit rubric and a mandatory `satisfaction_basis` evidence sentence; `null` when no evidence exists, never a default number. MineReflections/Upgrade consumers updated with a legacy-field caveat. Migration: `echo v6.3.4 > LATEST`. Rollback: `echo v6.3.3 > LATEST`.

- **v6.3.3** (2026-07-06) — **Self-selection alignment release (drift register W1.2).** Doctrine catches up with the suggestion posture the runtime ships since W1.2 (`a8beba6`): the stored mode/tier classification is a **suggestion** the executor adopts when it matches its own read and overrides (either direction) when it does not; the only binding tier signal is an explicit `/e1`–`/e5` from the Principal. Removes the executor-side obedience orders that contradicted that posture — "The executor honors the stored classification", override-hierarchy item 2 ("honor stored classifier output"), and OBSERVE's "use `TIER` verbatim" — and adds `effort_source: self-selected`. Documents that the registered `/eN` slash-commands arrive **expanded** ("Run PAI effort EN for: …") and that the classifier binds that phrase as an override too (runtime fix `6453d7c`; before it, the override branch never fired on the slash-command path). Classifier-side fail-safe stays ALGORITHM E3; it constrains the classifier's output, not the executor's judgment. Migration: `echo v6.3.3 > LATEST`. Rollback: `echo v6.3.2 > LATEST`.

- **v6.3.2** (2026-06-14, OpenCode follow-up 2026-06-15/16) — **OpenCode core runtime release.** Adds real long-memory tools to the OpenCode port: `PAI/TOOLS/MemoryRetriever.ts` for BM25-lite lexical retrieval over `MEMORY/KNOWLEDGE/`, `PAI/TOOLS/KnowledgeGraph.ts` for stats/find/related/traverse over tags, wikilinks, and typed `related:` links, `PAI/TOOLS/SessionHarvester.ts` for dry-run-first transcript mining, `PAI/TOOLS/KnowledgeHarvester.ts` for status/validate/index/contradictions/conservative harvest, and `PAI/MEMORY/README.md` as the runtime memory manifest. Adds OpenCode-native `CheckpointPerISC` behavior in the plugin with allowlist-only commits, idempotent sidecar state, and `PAI/TOOLS/Checkpoint.ts` rollback preview. Active doctrine uses installed tools when available and falls back to `rg`/manual reads when stale installs are missing them. The 2026-06-16 follow-up restores original-PAI classifier shape in the port: LLM-first mode/tier classification by default, deterministic heuristic only when LLM is explicitly disabled, LLM failure → ALGORITHM E3 fail-safe, and `opencode run --pure` to avoid recursive plugin execution. `LearningPatternSynthesis.ts` and Pulse memory consolidation remain deferred. Migration: `echo v6.3.2 > LATEST`. Rollback: `echo v6.3.1 > LATEST`.

- **v6.3.1** (2026-06-14) — **OpenCode port-coherence release.** No new doctrine primitive; this is a runtime-truth bump for the OpenCode port. Field audit found v6.3.0 and `PAI/CLAUDE.md` still promised Claude Code-only infrastructure: Sonnet classifier at `UserPromptSubmit`, `PromptProcessing.hook.ts` `additionalContext`, `PAI_SYSTEM_PROMPT.md` via `--append-system-prompt-file`, automatic `CheckpointPerISC.hook.ts` commits, kitty tab updates, and duplicated `~/.config/opencode/PAI/PAI/...` paths. Runtime inspection showed the port actually uses `pai-hooks.js` (`chat.message`, `experimental.chat.system.transform`, `tool.execute.*`, `permission.ask` adapter), a local heuristic classifier with optional LLM fallback, JSONL observability, explicit `pai_notify`, and no installed `PAI/TOOLS/Inference.ts` or CheckpointPerISC implementation. v6.3.1 makes the active doctrine honest: mode/tier is OpenCode plugin-driven; Advisor is unavailable until an installed `Inference.ts` or OpenCode-native adapter exists; CheckpointPerISC is unavailable until implemented; Cato/Forge/Anvil must honor structured unavailable/skipped behavior when helpers are missing; ISA sync updates registry/notifications, not kitty tabs; duplicated `PAI/PAI` paths are removed. Migration: `echo v6.3.1 > LATEST`. Rollback: `echo v6.3.0 > LATEST`.
...
v6.3.3 is the self-selection alignment correction. It does not change the core loop, ISA structure, tiers, floors, or the closed thinking-capability enumeration. It changes only the executor's relationship to the mode/tier classifier, closing the contradiction found by the 2026-07-05 adversarial audit of the `bitter` branch.

**C/R/L:**
- conjectured: softening `formatClassificationContext` (W1.2, `a8beba6`) was sufficient to make the classification advisory — the executor would follow the injected suggestion stance.
- refuted_by: the versioned doctrine the executor is told to "follow exactly" still ordered obedience in three places (executor paragraph, override hierarchy item 2, OBSERVE step 2 "use `TIER` verbatim"), giving the model contradictory instructions in the same session; and the `/eN` escape valve W1.2 depended on never fired on the slash-command path, because the registered commands expand to "Run PAI effort EN for: …" and `OVERRIDE_PATTERN` only matched a literal `/eN`.
- learned: a posture change must land on every instruction surface the model actually reads (injected context AND versioned doctrine AND phase steps), and every claimed escape valve needs an end-to-end guard exercising the exact string the runtime delivers, not the string the doctrine imagines.
- criterion_now: v6.3.3 contains no executor-side obedience order for non-`/eN` sources; `opencode/tests/mode-classifier.test.ts` binds the expanded template as `source: override` for every tier.

**What changed in the doctrine file:** header "suggested by … the executor owns the final call"; executor paragraph rewritten around adopt-or-override with `## Decisions` logging; override hierarchy item 2 reworded to suggestion; OBSERVE "Set effort level" gains the expanded-template note and `effort_source: self-selected`; fail-safe paragraph scoped to the classifier side.


### v6.3.0 -> v6.3.1

v6.3.1 is the OpenCode port-coherence correction. It does not change the core Algorithm loop, ISA structure, thinking floor, or closed thinking-capability enumeration. It changes the active doctrine's claims about the runtime so the executor stops relying on Claude Code infrastructure that is not present in this port.

**Runtime facts verified before the bump:**
- `opencode/plugins/pai-hooks.js` is the active runtime surface; it injects context via `experimental.chat.system.transform`, classifies via `chat.message`, guards via `tool.execute.before`, syncs state via `tool.execute.after`, and bridges OpenCode bus events through `hooks.event`.
- `opencode/plugins/lib/mode-classifier.lib.js` is heuristic by default, with optional LLM fallback, not a mandatory Sonnet `UserPromptSubmit` hook.
- `PAI/TOOLS/` is empty in the port, so Advisor/Inference and Checkpoint CLI promises cannot be treated as active.
- `CheckpointPerISC.hook.ts` is not implemented in the OpenCode plugin.
- `pai_notify` is the final voice contract; legacy `/notify` is broker compatibility, not a required Algorithm dependency.

**Files updated:**
- NEW: `PAI/ALGORITHM/v6.3.1.md`
- EDIT: `PAI/ALGORITHM/LATEST` -> `v6.3.1`
- EDIT: `PAI/ALGORITHM/changelog.md`
- EDIT: `PAI/CLAUDE.md`
...
- 2026-04-28 | conjectured: the existing skill-and-hook surface was sufficient infrastructure for the ISA primitive
  ...
  criterion now: new `~/.config/opencode/PAI/skills/ISA/` skill with Scaffold / Interview / CheckCompleteness / Reconcile / Seed / Append workflows, and Append specifically gates the C/R/L format (refuses partial entries)
...
- NEW: `~/.config/opencode/PAI/skills/ISA/` (eleven files: SKILL.md, four Examples, six Workflows)
- NEW: `~/.config/opencode/PAI/ISA.md` (dog-food: PAI-as-project ISA, seeded via the Seed workflow)
...
- EDIT: `~/.config/opencode/PAI/CLAUDE.md` (MANDATORY FIRST ACTION → v6.2.0.md)
...
- `~/.config/opencode/PAI/skills/ISA/Tools/*.ts` CLI implementations (Scaffold.ts, Interview.ts, Completeness.ts, Reconcile.ts, Append.ts)
...
```bash
echo 6.1.0 > ~/.config/opencode/PAI/PAI/ALGORITHM/LATEST && \
  jq '.pai.algorithmVersion="6.1.0"' ~/.config/opencode/PAI/settings.json | sponge ~/.config/opencode/PAI/settings.json && \
  sed -i '' 's|v6.2.0.md|v6.1.0.md|' ~/.config/opencode/PAI/CLAUDE.md
...
- EDIT: `~/.config/opencode/PAI/CLAUDE.md` (MANDATORY FIRST ACTION → v6.0.0.md)
- EDIT: `~/.config/opencode/PAI/settings.json` (`pai.algorithmVersion` → `6.0.0`, hook registration for new UserPromptSubmit + SessionStart entries)

**Migration command (rollback to v5.5.0):**
```bash
echo 5.5.0 > ~/.config/opencode/PAI/PAI/ALGORITHM/LATEST && \
  jq '.pai.algorithmVersion="5.5.0"' ~/.config/opencode/PAI/settings.json | sponge ~/.config/opencode/PAI/settings.json && \
  sed -i '' 's|v6.0.0.md|v5.5.0.md|' ~/.config/opencode/PAI/CLAUDE.md
...
- `~/.config/opencode/PAI/CLAUDE.md` (MANDATORY FIRST ACTION → v5.5.0.md)
- `~/.config/opencode/PAI/settings.json` (`pai.algorithmVersion` → `5.5.0`)

**Migration command (rollback):** `echo v5.4.0 > ~/.config/opencode/PAI/PAI/ALGORITHM/LATEST && jq '.pai.algorithmVersion="5.4.0"' ~/.config/opencode/PAI/settings.json | sponge ~/.config/opencode/PAI/settings.json && sed -i '' 's|v5.5.0.md|v5.4.0.md|' ~/.config/opencode/PAI/CLAUDE.md`. Hooks need no rollback — both detection paths (legacy `-A-` and new `Anti:` prefix) coexist permanently.
...
**Migration command (rollback):** `echo v5.2.0 > LATEST && jq '.pai.algorithmVersion="5.2.0"' ~/.config/opencode/PAI/settings.json | sponge ~/.config/opencode/PAI/settings.json && sed -i '' 's|v5.3.0.md|v5.2.0.md|' ~/.config/opencode/PAI/CLAUDE.md`. Hooks need no rollback — the parser still understands the old bracket format.
...
**Migration:** v5.2.0 lives alongside v5.1.0 and v5.0.0. To roll back: `echo v5.1.0 > LATEST && jq '.pai.algorithmVersion="5.1.0"' ~/.config/opencode/PAI/settings.json | sponge ~/.config/opencode/PAI/settings.json && sed -i '' 's|v5.2.0.md|v5.1.0.md|' ~/.config/opencode/PAI/CLAUDE.md`. Hooks unchanged (parse only `effort` enum + ISC checkboxes + category tags). Floors are doctrine-level only — `isa-utils.ts` does not enforce them; the Algorithm prompt does.

### v5.0.0 → v5.1.0

v5.1.0 adds **per-step durability** — the one design move PAI hadn't yet committed to. The `CheckpointPerISC` hook (PostToolUse Edit/Write/MultiEdit on `MEMORY/WORK/{slug}/ISA.md`) auto-commits a git checkpoint in every opted-in repo on each `[ ]`→`[x]` ISC transition. Commits use `--no-verify --no-gpg-sign` to avoid husky/GPG hangs that would block the session. Sidecar state at `MEMORY/WORK/{slug}/.checkpoint-state.json` makes commits idempotent (no double-commit). Allowlist at `~/.config/opencode/PAI/checkpoint-repos.txt` — defaults to `~/.claude` only; other repos require explicit {{PRINCIPAL_NAME}} opt-in (no surprise commits in WIP branches). Inspection + rollback via `bun ~/.config/opencode/PAI/PAI/TOOLS/Checkpoint.ts {list|show|rollback} <slug> [<isc-id>]`. **Rollback is preview-only** — prints the suggested `git reset --hard <sha>` per repo and exits; never executes destructive ops without {{PRINCIPAL_NAME}} running them himself. Doctrine elsewhere is unchanged from v5.0.0; the only addition is the **Checkpoints subsection** in EXECUTE below. Provenance: R1 from session `20260426-230039_hankweave-pai-improvements-analysis` — Hankweave's per-codon git checkpoint absorbed as a PAI-native primitive without adopting Hankweave's runtime.

**Migration:** v5.1.0 lives alongside v5.0.0. To roll back: `echo v5.0.0 > LATEST && jq '.pai.algorithmVersion="5.0.0"' ~/.config/opencode/PAI/settings.json | sponge ~/.config/opencode/PAI/settings.json && sed -i '' 's|v5.1.0.md|v5.0.0.md|' ~/.config/opencode/PAI/CLAUDE.md`. The hook can be left registered with no allowlist file (it no-ops); or remove its entries from `settings.json` PostToolUse arrays.
...
4. Update `LATEST` and `~/.config/opencode/PAI/CLAUDE.md`'s algorithm reference to the new version.
