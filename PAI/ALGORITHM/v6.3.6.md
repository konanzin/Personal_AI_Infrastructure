...
- **IterativeDepth** — multi-angle exploration; default at Extended+ when budget allows (scope/zoom oscillation is a 3-pass variant of this lens, not a separate name)
- **FeedbackMemoryConsult** — grep prior `feedback_*.md` for matching mistakes
- **Advisor** — commitment-boundary second-opinion via `PAI/TOOLS/Inference.ts`. Available only when this machine configures `PAI_INFERENCE_CMD` (see `PAI/.env`); if unconfigured, record `advisor_unavailable` and choose another implemented thinking capability to meet the floor.
...
- **RedTeam** — adversarial stress-test of ideas and plans
- **Science** — hypothesis-plural falsifiable experiments (incident/causal-chain investigation is this lens with contributing-factors framing)
...
(W2.15 skill census removed ApertureOscillation, RootCauseAnalysis, BeCreative, Ideate, WorldThreatModel, and Fabric patterns from this enumeration — their skills retired; divergent ideation and causal investigation are things the model does inline, not names to select.)
...
### Mode Classification (v6.3.6 OpenCode port)
...
- **Thinking floor (soft, v6.3.4)** — meet the tier thinking floor (E2 ≥2, E3 ≥4, E4 ≥6, E5 ≥8) or relax it with "show your math" in `## Decisions` naming what the un-selected thinking would have contributed; silent under-floor is a violation. Names MUST come verbatim from the closed enumeration (W2.15 census: IterativeDepth, FeedbackMemoryConsult, Advisor, ReReadCheck, FirstPrinciples, SystemsThinking, Council, RedTeam, Science, BitterPillEngineering, Evals, ContextSearch, ISA). Inventing generic thinking labels is a phantom capability and a CRITICAL FAILURE.
