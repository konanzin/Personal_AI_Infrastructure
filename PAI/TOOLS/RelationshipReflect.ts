...
// OpenCode: paths resolve under the PAI dir (USER/ and MEMORY/ both live here),
// unlike the original which split MEMORY at the .claude root.
const PAI_DIR = getPaiDir();

// Quiet mode for --json output (suppresses human log lines on stdout).
let QUIET = false;
...
async function reflect(options: {
  ...
  say(`Loaded ${notes.length} relationship notes from last 7 days`);
  say(`Loaded ${ratings.length} ratings from last 7 days`);
...
`);
  ...
  const opinionsPath = join(PAI_DIR, 'USER/OPINIONS.md');
  const storyPath = join(PAI_DIR, 'USER/OUR_STORY.md');
  const hasSource = existsSync(relationshipDir) || existsSync(opinionsPath) || existsSync(storyPath);
  if (!hasSource) {
    if (asJson) {
      console.log(JSON.stringify({ status: 'unavailable', reason: 'No relationship sources (MEMORY/RELATIONSHIP, OPINIONS.md, OUR_STORY.md)' }, null, 2));
    } else {
      console.log('RelationshipReflect unavailable: no relationship sources found.');
    }
    process.exit(0);
  }

  const result = await reflect(options);

  if (asJson) {
    console.log(JSON.stringify({
      status: 'ok',
      dry_run: !!options.dryRun,
      opinions_updated: result.opinionsUpdated,
      major_shifts: result.majorShifts,
      milestones: result.milestonesDetected,
      soul_updates_queued: result.soulUpdatesQueued,
    }, null, 2));
  }
}
