...
> **Infrastructure:** The voice notification endpoint (`http://localhost:31337/notify`) is served by the unified Pulse daemon (`~/.config/opencode/PAI/PAI/PULSE/`). Voice is implemented at `~/.config/opencode/PAI/PAI/PULSE/VoiceServer/voice.ts` and routed through Pulse -- there is no separate VoiceServer process. One daemon, one port, one launchd plist (`com.pai.pulse`).
...
**Full voice registry:** `~/.config/opencode/PAI/skills/Agents/SKILL.md` (see Named Agents) and `~/.config/opencode/PAI/settings.json` (daidentity.voices.main.voiceId)
...
Located in `~/.config/opencode/PAI/settings.json`:
...
The notification service is in `~/.config/opencode/PAI/hooks/lib/notifications.ts`:
...
Events are emitted via `~/.config/opencode/PAI/hooks/lib/observability-transport.ts`, which is synchronous and fire-and-forget. This channel is additive -- it does not replace any of the notification channels above, and hooks emit events alongside their existing state writes and notifications.
