...
- Can reference `~/.config/opencode/PAI/PAI/USER/SKILLCUSTOMIZATIONS/<SkillName>/` at runtime for per-user tweaks
...
- The decision rule: *"Could this skill be dropped, as-is, into a stranger's `~/.config/opencode/PAI/skills/` and just work?"* If no, it must be `_ALLCAPS`
...
`~/.config/opencode/PAI/PAI/USER/SKILLCUSTOMIZATIONS/{SkillName}/`
...
```
~/.config/opencode/PAI/PAI/USER/SKILLCUSTOMIZATIONS/
...
1. **Create directory**: `mkdir -p ~/.config/opencode/PAI/PAI/USER/SKILLCUSTOMIZATIONS/SkillName`
...
**See:** `~/.config/opencode/PAI/skills/Science/Protocol.md` for the full protocol interface
...
```
~/.config/opencode/PAI/skills/CreateSkill/Workflows/CanonicalizeSkill.md
```
...
```
~/.config/opencode/PAI/skills/CreateSkill/Workflows/TestSkill.md
```
...
**Reference:** any well-formed skill in `~/.config/opencode/PAI/skills/` follows the same pattern; private personal skills use the `_NAME/` form below.
...
**See:** `~/.config/opencode/PAI/PAI/DOCUMENTATION/Tools/CliFirstArchitecture.md` (Workflow-to-Tool Integration section)
 ...
 *
 * Usage:
 *   bun ~/.config/opencode/PAI/skills/SkillName/Tools/ToolName.ts <command> [options]
 *
 * Commands:
