...
**Location:** `~/.config/opencode/PAI/PAI/TOOLS/Inference.ts`
...
bun ~/.config/opencode/PAI/PAI/TOOLS/Inference.ts --level fast "System prompt" "User prompt"

# Standard (Sonnet) - balanced reasoning, typical analysis
bun ~/.config/opencode/PAI/PAI/TOOLS/Inference.ts --level standard "System prompt" "User prompt"

# Smart (Opus) - deep reasoning, strategic decisions
bun ~/.config/opencode/PAI/PAI/TOOLS/Inference.ts --level smart "System prompt" "User prompt"

# With JSON output
bun ~/.config/opencode/PAI/PAI/TOOLS/Inference.ts --json --level fast "Return JSON" "Input"

# Custom timeout
bun ~/.config/opencode/PAI/PAI/TOOLS/Inference.ts --level standard --timeout 60000 "Prompt" "Input"
```
...
**Programmatic Usage:**
```typescript
// From hooks (at ~/.config/opencode/PAI/hooks/):
...
**Location:** `~/.config/opencode/PAI/PAI/TOOLS/RemoveBg.ts`
...
bun ~/.config/opencode/PAI/PAI/TOOLS/RemoveBg.ts /path/to/image.png

# Remove background and save to different path
bun ~/.config/opencode/PAI/PAI/TOOLS/RemoveBg.ts /path/to/input.png /path/to/output.png

# Process multiple images
bun ~/.config/opencode/PAI/PAI/TOOLS/RemoveBg.ts image1.png image2.png image3.png
```
...
**Location:** `~/.config/opencode/PAI/PAI/TOOLS/AddBg.ts`
...
bun ~/.config/opencode/PAI/PAI/TOOLS/AddBg.ts /path/to/transparent.png "#EAE9DF" /path/to/output.png

# Add your brand background color (uses the color from PAI config)
bun ~/.config/opencode/PAI/PAI/TOOLS/AddBg.ts /path/to/transparent.png --brand /path/to/output.png
```
...
**Location:** `~/.config/opencode/PAI/PAI/TOOLS/GetTranscript.ts`
...
bun ~/.config/opencode/PAI/PAI/TOOLS/GetTranscript.ts "https://www.youtube.com/watch?v=VIDEO_ID"

# Save transcript to file
bun ~/.config/opencode/PAI/PAI/TOOLS/GetTranscript.ts "https://www.youtube.com/watch?v=VIDEO_ID" --save /path/to/transcript.txt
```
...
**Technical Details:**
- Pulse must be running (voice handler lives at `~/.config/opencode/PAI/PAI/PULSE/VoiceServer/voice.ts`, port 31337)
...
**Location:** `~/.config/opencode/PAI/PAI/TOOLS/extract-transcript.py`
...
cd ~/.config/opencode/PAI/PAI/TOOLS/
...
**Location:** `~/.config/opencode/PAI/PAI/TOOLS/YouTubeApi.ts`
...
bun ~/.config/opencode/PAI/PAI/TOOLS/YouTubeApi.ts --channel-stats

# Get video statistics
bun ~/.config/opencode/PAI/PAI/TOOLS/YouTubeApi.ts --video-stats VIDEO_ID

# Get latest uploads
bun ~/.config/opencode/PAI/PAI/TOOLS/YouTubeApi.ts --latest-videos
```

**Environment Variables:**
- `YOUTUBE_API_KEY` - Required for API access (from `~/.config/opencode/PAI/.env`)
...
Monitor({
  ...
  persistent: true,
  timeout_ms: 3600000,
  command: "bun $HOME/.config/opencode/PAI/PAI/TOOLS/AgentWatchdog.ts"
})
```
...
Monitor({
  ...
  persistent: true,
  timeout_ms: 300000,
  command: "tail -f ~/.config/opencode/PAI/Pulse/logs/pulse-stdout.log | grep --line-buffered -i -E '(error|fatal|crash|unhandled)'"
})
```
...
1. **Add tool file:** Place `.ts` or `.py` file directly in `~/.config/opencode/PAI/PAI/TOOLS/`
...
2. **Document here:** Add section to this file with:
   - Tool location (e.g., `~/.config/opencode/PAI/`)
   - Usage examples
...
**Location:** `~/.config/opencode/PAI/PAI/TOOLS/algorithm.ts`
...
**Parameter schema:** `~/.config/opencode/PAI/PAI/ALGORITHM/parameter-schema.md`

---

## AlgorithmPhaseReport.ts - Algorithm State Reporter

**Location:** `~/.config/opencode/PAI/PAI/TOOLS/AlgorithmPhaseReport.ts`
...
**Location:** `~/.config/opencode/PAI/PAI/TOOLS/ArchitectureSummaryGenerator.ts`
...
bun ~/.config/opencode/PAI/PAI/TOOLS/ArchitectureSummaryGenerator.ts generate

# Check if summary is stale (exit 1 if stale, 0 if fresh)
bun ~/.config/opencode/PAI/PAI/TOOLS/ArchitectureSummaryGenerator.ts check
```
...
**Config:** `~/.config/opencode/PAI/.mcp.json`
...
- **Architecture**: `~/.config/opencode/PAI/PAI/DOCUMENTATION/PAISystemArchitecture.md` (master architecture reference)
- **CLI Tools**: `~/.config/opencode/PAI/PAI/DOCUMENTATION/Tools/Cli.md` (Algorithm CLI, Arbol CLI)
