...
2. Walk `~/.config/opencode/PAI/` at depth 1-2 (e.g. `ls -la && ls -la PAI/ && ls -la skills/`) and compare against the zone list.
...
3. Fallback path: read from `~/.config/opencode/PAI/.env` via the shared `readEnvOrPaiEnv()` helper (see `hooks/lib/observability-transport.ts`). Legacy `PAI/.env` symlink has been retired; the authoritative file is `~/.config/opencode/PAI/.env`.
...
6. **Public publish is a separate step.** The shadow release stays under `PAI/PAI_RELEASES/PAI_Release_v{VERSION}/.config/opencode/PAI/` until a deliberate publish action ships it to the public repo.
...
| `agents/<agent>.md` | Write-permission path literals in agent definitions | **TODO-REFACTOR** — verify env-expansion support in Claude Code agent spec, then replace with `${HOME}/.config/opencode/PAI/...` |
