...
bun ~/.config/opencode/PAI/PAI/TOOLS/algorithm.ts -m loop -p <ISA-path> -n 20

# Run in interactive mode (launches a claude session with ISA context)
bun ~/.config/opencode/PAI/PAI/TOOLS/algorithm.ts -m interactive -p <ISA-path>

# Run in optimize mode (launches claude with /optimize arguments)
bun ~/.config/opencode/PAI/PAI/TOOLS/algorithm.ts -m optimize -p <ISA-path>
...
bun ~/.config/opencode/PAI/PAI/TOOLS/algorithm.ts status
```
...
bun ~/.config/opencode/PAI/PAI/TOOLS/algorithm.ts -m loop -p ISA-20260213-auth.md

# Fast loop — 20 max iterations
bun ~/.config/opencode/PAI/PAI/TOOLS/algorithm.ts -m loop -p ISA-20260213-auth.md -n 20

# Parallel loop — 4 agents working on different criteria simultaneously
bun ~/.config/opencode/PAI/PAI/TOOLS/algorithm.ts -m loop -p ISA-20260213-auth.md -n 20 -a 4
```
...
```bash
bun ~/.config/opencode/PAI/PAI/TOOLS/algorithm.ts -m interactive -p ISA-20260213-feature.md
```
...
bun ~/.config/opencode/PAI/PAI/TOOLS/algorithm.ts status -p ISA-20260213-auth

# Pause a running loop (loop checks between iterations)
bun ~/.config/opencode/PAI/PAI/TOOLS/algorithm.ts pause -p ISA-20260213-auth

# Resume a paused loop
bun ~/.config/opencode/PAI/PAI/TOOLS/algorithm.ts resume -p ISA-20260213-auth

# Stop a loop permanently
bun ~/.config/opencode/PAI/PAI/TOOLS/algorithm.ts stop -p ISA-20260213-auth
```
...
**Location:** `~/.config/opencode/PAI/PAI/ARBOL/Actions/pai.ts`
...
```bash
cd ~/.config/opencode/PAI/PAI/ARBOL/Actions
...
**Location:** `~/.config/opencode/PAI/PAI/ARBOL/Actions/lib/runner.v2.ts`
...
**Location:** `~/.config/opencode/PAI/PAI/ARBOL/Actions/lib/pipeline-runner.ts`
...
```bash
# The Algorithm CLI
alias algorithm="bun ~/.config/opencode/PAI/PAI/TOOLS/algorithm.ts"

# The Arbol CLI
alias pai="bun ~/.config/opencode/PAI/PAI/ARBOL/Actions/pai.ts"

# Runners (optional — pai CLI wraps these)
alias arbol-run="bun ~/.config/opencode/PAI/PAI/ARBOL/Actions/lib/runner.v2.ts"
alias arbol-pipe="bun ~/.config/opencode/PAI/PAI/ARBOL/Actions/lib/pipeline-runner.ts"
```
