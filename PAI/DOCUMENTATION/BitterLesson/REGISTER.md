...
| W2.9 | Counts mágicos nos validators; **fonte duplicada** `PAI/bin/*` vs `opencode/bin/*` (descoberta 2026-07-06: as cópias `PAI/bin/` — as que o installer deploya — ficaram com o pin do deepseek e as asserções pré-W1.1a/b; sincronizadas em caráter de correção, fonte única fica para este workstream) | `validate-pai-installation.sh`, `test-behavioral.sh` (×2 cópias) | checagem de contrato, não de contagem; UMA fonte por validator | pendente |
...
| W2.11 | Context Routing Table ~48 linhas topic→path, já driftando (violação #24 do relatório, órfã até 2026-07-06) | `PAI/CLAUDE.md` §Context Routing | descobrir paths via filesystem/registry; manter só a curadoria que o filesystem não expressa (canonical-vs-legacy) | **pendente** |
| W2.12 | Census `PAI/TOOLS/` — a camada implementada de memória/aprendizado ficou FORA do inventário original (auditoria adversarial 2026-07-06). Padrões-alvo em densidade máxima: `RelationshipReflect.ts` regexes de evidência + deltas mágicos (+0.02/−0.05/+0.10/−0.20, limiar 0.15) **mutando confidences persistidas em USER/OPINIONS.md** (pior que W1.8 — a fabricação persiste); `SessionHarvester.ts:268` confidence fabricada por fórmula; `MigrateScan.ts` tabela de ~40 keywords + confidence mágica (mitigado por fila de aprovação); `LearningPatternSynthesis.ts` recomendações enlatadas por bucket; `KnowledgeHarvester.ts` roteamento de domínio por keyword | auditar cada tool com o gate de 3 perguntas; prioridade 1 = RelationshipReflect (bloquear escrita não-dry-run até o veredito) | **pendente** |
