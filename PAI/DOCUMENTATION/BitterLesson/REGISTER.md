...
| W1.2 | "honor above self-selection" do classificador | `mode-classifier.lib.js` formatClassificationContext; `opencode.jsonc.template` | doutrina upstream v6.3.0 (incidente de sub-escalação, abr/2026) | classificação vira sugestão; `/eN` do usuário segue autoritativo | **feito** `a8beba6` — golden heurístico inalterado 68%/71%; pendente: golden LLM (config de produção) antes de mesclar em dori |
