...
**Location:** `~/.config/opencode/PAI/PAI/PULSE/`
...
Run a shell command via `bash -c`. The working directory is `~/.config/opencode/PAI/Pulse/`. Environment variables from `~/.config/opencode/PAI/.env` are available. The process has a 60-second timeout (SIGTERM on expiry).
...
Located at `~/.config/opencode/PAI/Pulse/state/state.json`. Written atomically (write to `.tmp`, rename) after each job execution.
...
| `WorkingDirectory` | `~/.config/opencode/PAI/Pulse` | CWD for the process |

Logs go to `~/.config/opencode/PAI/Pulse/logs/pulse-stdout.log` and `pulse-stderr.log`.
...
3. Restart Pulse: `~/.config/opencode/PAI/Pulse/manage.sh restart`
...
```bash
~/.config/opencode/PAI/Pulse/manage.sh status
```
...
tail -50 ~/.config/opencode/PAI/Pulse/logs/pulse-stdout.log

# Recent errors
tail -50 ~/.config/opencode/PAI/Pulse/logs/pulse-stderr.log

# Follow live
tail -f ~/.config/opencode/PAI/Pulse/logs/pulse-stdout.log | bun -e "process.stdin.on('data', d => { try { const e = JSON.parse(d); console.log(e.ts, e.level, e.msg) } catch {} })"
```
...
| "Telegram dispatch skipped" | Missing env vars | Set `TELEGRAM_BOT_TOKEN` and `TELEGRAM_PRINCIPAL_CHAT_ID` in `~/.config/opencode/PAI/.env` |
| "ntfy dispatch skipped" | Missing env var | Set `NTFY_TOPIC` in `~/.config/opencode/PAI/.env` |
...
| Calendar returns NO_EVENTS always | Missing or expired refresh token | Set `GOOGLE_CALENDAR_REFRESH_TOKEN` in `~/.config/opencode/PAI/.env` |
...
**Location:** `~/.config/opencode/PAI/PAI/PULSE/MenuBar/`
...
```bash
cd ~/.config/opencode/PAI/PAI/PULSE/MenuBar
...
The hooks are configured in `~/.config/opencode/PAI/settings.json` as HTTP hooks pointing to `http://localhost:31337/hooks/*`.
...
```bash
cd ~/.config/opencode/PAI/PAI/Observability && bun run build
