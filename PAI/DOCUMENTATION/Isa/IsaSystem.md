...
- **Skill** — `~/.config/opencode/PAI/skills/ISA/SKILL.md` — the workflows that generate, refine, score, and merge ISAs (Scaffold, Interview, CheckCompleteness, Reconcile, Seed, Append).
...
The ISA skill at `~/.config/opencode/PAI/skills/ISA/` owns six workflows. They map verb-in-the-request to a deterministic procedure.
...
- **Skills** (`PAI/DOCUMENTATION/Skills/SkillSystem.md`) — the ISA skill is one skill among many; it follows the same canonical form (TitleCase directory for public, `_ALLCAPS` for private; `Workflows/` + optional `Tools/` + optional `Examples/`; optional legacy Pulse progress-notification block; mandatory Gotchas section).
...
- **CreateSkill** (`~/.config/opencode/PAI/skills/CreateSkill/SKILL.md`) — the public-skill content rule that the ISA skill itself must obey. Public skills like ISA are stranger-safe; private skills like `<your-release-skill>` carry identity-bound content.
- **Pulse** (`PAI/DOCUMENTATION/Pulse/PulseSystem.md`) — in the OpenCode port, ISA phase changes emit notifications and observability events that an optional Pulse broker/renderer may display. The original dashboard phase widget is legacy desktop Pulse scope.
...
- **Skill (workflow implementations):** `~/.config/opencode/PAI/skills/ISA/SKILL.md`
- **Skill canonical example:** `~/.config/opencode/PAI/skills/ISA/Examples/canonical-isa.md`
