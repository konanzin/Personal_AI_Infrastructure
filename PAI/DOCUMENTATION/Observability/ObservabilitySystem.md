...
> **Infrastructure:** The observability HTTP server (`localhost:31337`) runs as a module inside the unified Pulse daemon (`~/.config/opencode/PAI/PAI/PULSE/Observability/observability.ts`). There is no separate observability server process -- Pulse serves all local HTTP endpoints on port 31337.
...
The `observability` section in `~/.config/opencode/PAI/settings.json` controls where events are pushed:
...
|------|-----------|------|----------|
| `cloudflare-kv` | CF KV API PUT to namespace | `CLOUDFLARE_API_TOKEN_WORKERS_EDIT` or `CLOUDFLARE_API_TOKEN` from `~/.config/opencode/PAI/.env` (tried in that order; env var wins over .env) | Production dashboards on Cloudflare Workers |
...
| File | Role |
|------|------|
| `~/.config/opencode/PAI/hooks/lib/observability-transport.ts` | `collectEvents()` + `pushEventsToTargets()` — the core pipeline |
| `~/.config/opencode/PAI/hooks/lib/identity.ts` | `ObservabilityTarget` type + `getObservabilityConfig()` — reads settings.json |
| `~/.config/opencode/PAI/hooks/ToolActivityTracker.hook.ts` | PostToolUse catch-all — writes JSONL + triggers KV push |
| `~/.config/opencode/PAI/hooks/KVSync.hook.ts` | SessionEnd — batch pushes events + state to all targets |
| `~/.config/opencode/PAI/PAI/PULSE/Observability/observability.ts` | Observability module inside unified Pulse daemon — serves events from JSONL at :31337 |
| `~/.config/opencode/PAI/PAI/PULSE/Observability/` | Next.js static dashboard — polls `/api/events/recent` |
| `~/.config/opencode/PAI/settings.json → observability` | Target configuration — add/remove destinations here |
...
| Item | Value |
|------|-------|
| Source | `~/.config/opencode/PAI/PAI/PULSE/Observability/` |
| Build command | `cd ~/.config/opencode/PAI/PAI/PULSE/Observability && bun run build` (outputs to `out/`) |
| Serving mechanism | Direct: `~/.config/opencode/PAI/PAI/PULSE/Observability/out` (configured in PULSE.toml `dashboard_dir`) |
...
1. Edit source in `~/.config/opencode/PAI/PAI/PULSE/Observability/src/`
2. Build: `cd ~/.config/opencode/PAI/PAI/PULSE/Observability && bun run build`
...
- `~/.config/opencode/PAI/PAI/DOCUMENTATION/PAISystemArchitecture.md` — Master PAI architecture reference
