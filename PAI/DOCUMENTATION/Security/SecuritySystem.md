...
1. **Trusted workspace** (fast path, no LLM): `~/.config/opencode/PAI/`, `~/Projects/`, `~/LocalProjects/` → auto-approve
...
### Hook Entry Points (`~/.config/opencode/PAI/hooks/`)
| File | Event | Purpose |
|------|-------|---------|
...
### Pipeline Core (`~/.config/opencode/PAI/hooks/security/`)
| File | Purpose |
|------|---------|
...
### Inspectors (`~/.config/opencode/PAI/hooks/security/inspectors/`)
...
### Policy Files (`~/.config/opencode/PAI/PAI/USER/SECURITY/`)
| File | Purpose |
|------|---------|
...
**Deployment:**
```bash
cd ~/.config/opencode/PAI/PAI/PULSE/Observability && bun run build
