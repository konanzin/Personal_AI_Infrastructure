...
| **SmartApprover.hook.ts** | PermissionRequest | Auto-approve or ask | Trusted workspace paths (~/.config/opencode/PAI/, ~/Projects/, ~/LocalProjects/) auto-approve. Non-trusted paths classified as read (auto-approve) or write (prompt user) via haiku. Permission cache avoids repeat LLM calls. |
...
- `~/.config/opencode/PAI/PAI/`
- `~/.config/opencode/PAI/PAI/hooks/`
- `~/.config/opencode/PAI/PAI/skills/`

This means the PatternInspector cannot protect PAI infrastructure files from being overwritten. The `noDelete` rule for `~/.config/opencode/PAI/PAI/**` only applies to the Delete path action, and the fast-path bypass runs before `noDelete` is checked. (EgressInspector and RulesInspector still run on these paths.)
...
1. **Trusted workspace paths** (~/.config/opencode/PAI/, ~/Projects/, ~/LocalProjects/) → auto-approve immediately. No LLM call.
