...
> **legacy/reference material** — This document describes the original Claude Code hook architecture. The active OpenCode port uses `opencode/plugins/pai-hooks.js` plus `opencode/plugins/lib/*.js`; `.hook.ts` files and `settings.json` hook wiring are not active runtime surfaces unless a section is explicitly marked OpenCode-native.

> **PAI 5.0** — Stable event-driven automation infrastructure.

**Event-Driven Automation Infrastructure**

**Location:** `~/.config/opencode/PAI/hooks/`
**Configuration:** `~/.config/opencode/PAI/settings.json`
...
- **History Capture** - Automatic work/learning documentation to `~/.config/opencode/PAI/PAI/MEMORY/`
...
**Key Principle:** Most hooks run asynchronously and fail gracefully. Security hooks (SecurityPipeline) are synchronous and blocking — they can prevent tool execution via `exit(2)`. All `.ts` hooks have `#!/usr/bin/env bun` shebangs and `+x` permissions — settings.json references them directly (e.g., `$HOME/.config/opencode/PAI/hooks/SecurityPipeline.hook.ts`) without a `bun` prefix. HTTP hooks (SkillGuard, AgentGuard) run via Pulse routes on `localhost:31337`.
  ...
  "SessionStart": [
    {
      "hooks": [
        {
          "type": "command",
          "command": "$HOME/.config/opencode/PAI/hooks/KittyEnvPersist.hook.ts"
        },
        {
          "type": "command",
          "command": "$HOME/.config/opencode/PAI/hooks/LoadContext.hook.ts"
        },
        {
          "type": "command",
          "command": "$HOME/.config/opencode/PAI/hooks/KVSync.hook.ts",
          "timeout": 10,
          "async": true
        }
      ]
  ...
  "SessionEnd": [
    {
      "hooks": [
        {
          "type": "command",
          "command": "$HOME/.config/opencode/PAI/hooks/WorkCompletionLearning.hook.ts"
        },
        {
          "type": "command",
          "command": "$HOME/.config/opencode/PAI/hooks/ULWorkSync.hook.ts"
        },
        {
          "type": "command",
          "command": "$HOME/.config/opencode/PAI/hooks/SessionCleanup.hook.ts"
        },
        {
          "type": "command",
          "command": "$HOME/.config/opencode/PAI/hooks/RelationshipMemory.hook.ts"
        },
        {
          "type": "command",
          "command": "$HOME/.config/opencode/PAI/hooks/UpdateCounts.hook.ts"
        },
        {
          "type": "command",
          "command": "$HOME/.config/opencode/PAI/hooks/IntegrityCheck.hook.ts"
        },
        {
          "type": "command",
          "command": "$HOME/.config/opencode/PAI/hooks/KVSync.hook.ts"
  ...
  "UserPromptSubmit": [
    {
      "hooks": [
        {
          "type": "command",
          "command": "$HOME/.config/opencode/PAI/hooks/PromptGuard.hook.ts",
          "timeout": 10
        }
      ]
    },
    {
      "hooks": [
        {
          "type": "command",
          "command": "$HOME/.config/opencode/PAI/hooks/PromptProcessing.hook.ts",
          "timeout": 30,
          "async": true
        }
      ]
    },
    {
      "hooks": [
        {
          "type": "command",
          "command": "$HOME/.config/opencode/PAI/hooks/RepeatDetection.hook.ts",
          "timeout": 5
        }
      ]
    },
    {
      "hooks": [
        {
          "type": "command",
          "command": "$HOME/.config/opencode/PAI/hooks/SatisfactionCapture.hook.ts",
          "timeout": 20,
          "async": true
        }
      ]
...
- **Inference:** `import { inference } from "~/.config/opencode/PAI/PAI/TOOLS/Inference.ts"` → Sonnet level
...
```json
{
  "Stop": [
    {
      "hooks": [
        { "type": "command", "command": "$HOME/.config/opencode/PAI/hooks/LastResponseCache.hook.ts" },
        { "type": "command", "command": "$HOME/.config/opencode/PAI/hooks/ResponseTabReset.hook.ts" },
        { "type": "command", "command": "$HOME/.config/opencode/PAI/hooks/VoiceCompletion.hook.ts" },
        { "type": "command", "command": "$HOME/.config/opencode/PAI/hooks/DocIntegrity.hook.ts" }
      ]
    }
  ]
}
  ...
  "PreToolUse": [
    {
      "matcher": "Bash",
      "hooks": [
        { "type": "command", "command": "$HOME/.config/opencode/PAI/hooks/SecurityPipeline.hook.ts" },
        { "type": "command", "command": "$HOME/.config/opencode/PAI/hooks/ContextReduction.hook.sh" }
      ]
    },
    {
    ...
    },
    {
      ...
      "hooks": [
        { "type": "command", "command": "$HOME/.config/opencode/PAI/hooks/SetQuestionTab.hook.ts" }
      ]
    }
  ]
}
...
```json
{
  "PostToolUse": [
    {
      "hooks": [
        { "type": "command", "command": "$HOME/.config/opencode/PAI/hooks/ContentScanner.hook.ts" }
      ]
    },
    {
      "matcher": "AskUserQuestion",
      "hooks": [
        { "type": "command", "command": "$HOME/.config/opencode/PAI/hooks/QuestionAnswered.hook.ts" }
      ]
    },
    {
      "matcher": "Write",
      "hooks": [
        { "type": "command", "command": "$HOME/.config/opencode/PAI/hooks/ISASync.hook.ts" },
        { "type": "command", "command": "$HOME/.config/opencode/PAI/hooks/TelosSummarySync.hook.ts" }
      ]
    },
    {
      "matcher": "Edit",
    ...
    {
      "hooks": [
        { "type": "command", "command": "$HOME/.config/opencode/PAI/hooks/ToolActivityTracker.hook.ts" }
      ]
    }
  ]
}
  ...
  "PostToolUseFailure": [
    {
      "hooks": [
        {
          "type": "command",
          "command": "$HOME/.config/opencode/PAI/hooks/ToolFailureTracker.hook.ts"
        }
      ]
    }
  ]
  ...
  "ConfigChange": [
    {
      "hooks": [
        {
          "type": "command",
          "command": "$HOME/.config/opencode/PAI/hooks/ConfigAudit.hook.ts"
        }
      ]
    }
  ]
  ...
  "PreCompact": [
    {
      ...
      "hooks": [
        {
          "type": "command",
          "command": "$HOME/.config/opencode/PAI/hooks/PreCompact.hook.ts"
        }
      ]
    }
  ]
  ...
  "PostCompact": [
    {
      "hooks": [
        {
          "type": "command",
          "command": "$HOME/.config/opencode/PAI/hooks/RestoreContext.hook.ts"
        }
      ]
    }
  ]
  ...
  "TeammateIdle": [
    {
      "hooks": [
        {
          "type": "command",
          "command": "$HOME/.config/opencode/PAI/hooks/TeammateIdle.hook.ts"
        }
      ]
    }
  ]
  ...
  "TaskCreated": [
    {
      "hooks": [
        {
          "type": "command",
          "command": "$HOME/.config/opencode/PAI/hooks/TaskGovernance.hook.ts"
        }
      ]
    }
  ]
  ...
  "StopFailure": [
    {
      "hooks": [
        {
          "type": "command",
          "command": "$HOME/.config/opencode/PAI/hooks/StopFailureHandler.hook.ts"
        }
      ]
    }
  ]
  ...
  "Elicitation": [
    {
      "hooks": [
        {
          "type": "command",
          "command": "$HOME/.config/opencode/PAI/hooks/ElicitationHandler.hook.ts"
        }
      ]
    }
  ]
  ...
  "FileChanged": [
    {
      "hooks": [
        {
          "type": "command",
          "command": "$HOME/.config/opencode/PAI/hooks/FileChanged.hook.ts"
        }
      ]
    }
  ]
  ...
  "InstructionsLoaded": [
    {
      "hooks": [
        {
          "type": "command",
          "command": "$HOME/.config/opencode/PAI/hooks/InstructionsLoadedHandler.hook.ts"
        }
      ]
    }
  ]
...
### Location
**File:** `~/.config/opencode/PAI/settings.json`
**Section:** `"hooks": { ... }`

### Environment Variables
Hooks have access to all environment variables from `~/.config/opencode/PAI/settings.json` `"env"` section:
    ...
    "HookEventName": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "$HOME/.config/opencode/PAI/hooks/my-hook.ts --arg value"
          }
        ]
      }
    ]
...
**Session Mapping:** `~/.config/opencode/PAI/PAI/MEMORY/STATE/agent-sessions.json`
```json
{
...
**Full Documentation:** See `~/.config/opencode/PAI/PAI/DOCUMENTATION/Pulse/TerminalTabs.md`
...
```bash
chmod +x ~/.config/opencode/PAI/hooks/my-custom-hook.ts
```
> **Note:** Not needed when using the `bun` prefix in settings.json — all PAI hooks use `bun $HOME/.config/opencode/PAI/hooks/...` which doesn't require the execute bit.
    ...
    "Stop": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "$HOME/.config/opencode/PAI/hooks/my-custom-hook.ts"
          }
        ]
      }
    ]
...
```bash
# Test hook directly
echo '{"session_id":"test","transcript_path":"/tmp/test.jsonl","hook_event_name":"Stop"}' | bun ~/.config/opencode/PAI/hooks/my-custom-hook.ts
```
...
**Check:**
1. Is hook script executable? `chmod +x ~/.config/opencode/PAI/hooks/my-hook.ts` (not needed when using `bun` prefix — all PAI hooks use `bun` prefix)
2. Is path correct in settings.json? Use `bun $HOME/.config/opencode/PAI/hooks/...`
3. Is settings.json valid JSON? `jq . ~/.config/opencode/PAI/settings.json`
4. Did you restart Claude Code after editing settings.json?

**Debug:**
```bash
# Test hook directly
echo '{"session_id":"test","transcript_path":"/tmp/test.jsonl","hook_event_name":"Stop"}' | bun ~/.config/opencode/PAI/hooks/my-hook.ts

# Check hook logs (stderr output)
tail -f ~/.config/opencode/PAI/hooks/debug.log  # If you add logging
```
...
4. Is ElevenLabs API key in `~/.config/opencode/PAI/.env`?
...
**Check:**
1. Does `~/.config/opencode/PAI/PAI/MEMORY/` directory exist?
2. Does current-work file exist? Check `~/.config/opencode/PAI/`
3. Is hook actually running? Check `~/.config/opencode/PAI/PAI/MEMORY/RAW/` for events
4. File permissions? `ls -la ~/.config/opencode/PAI/PAI/MEMORY/WORK/`
...
ls -lt ~/.config/opencode/PAI/PAI/MEMORY/WORK/ | head -10
ls -lt ~/.config/opencode/PAI/PAI/MEMORY/LEARNING/$(date +%Y-%m)/ | head -10

# Check raw events
tail ~/.config/opencode/PAI/PAI/MEMORY/RAW/$(date +%Y-%m)/$(date +%Y-%m-%d)_all-events.jsonl
```
...
**Check:**
1. Is `~/.config/opencode/PAI/PAI/MEMORY/STATE/agent-sessions.json` writable?
...
cat ~/.config/opencode/PAI/PAI/MEMORY/STATE/agent-sessions.json | jq .

# Check subagent-stop debug log
tail -f ~/.config/opencode/PAI/hooks/subagent-stop-debug.log
```
...
grep '"type":"user"' "$(ls -d ~/.config/opencode/PAI/projects/*/ | head -1)"*.jsonl | head -1 | jq '.type'
...
**Check:**
1. Does `~/.config/opencode/PAI/CLAUDE.md` exist?
...
bun ~/.config/opencode/PAI/hooks/LoadContext.hook.ts
...
- Permission denied → `chmod +x ~/.config/opencode/PAI/hooks/LoadContext.hook.ts` (not needed when using `bun` prefix — all PAI hooks use `bun` prefix)
...
```json
{
  "Stop": [
    {
      "hooks": [
        { "command": "$HOME/.config/opencode/PAI/hooks/VoiceCompletion.hook.ts" }  // Example: one of several Stop hooks
      ]
    }
  ]
}
...
USER PROMPT SUBMIT (4 hooks):
  ...
  # Browser-verify rule lives in RUNTIME_CONSTITUTION.md / CLAUDE.md ("Browser-verify all web
...
KEY FILES:
~/.config/opencode/PAI/settings.json              Hook configuration (GENERATED by ConfigRenderer — read, don't hand-edit)
~/.config/opencode/PAI/PAI_CONFIG.yaml            Source of truth for config (ConfigRenderer reads this)
~/.config/opencode/PAI/TOOLS/ConfigRenderer.ts  Legacy renderer for settings/CLAUDE-era config; OpenCode runtime uses opencode.jsonc, CLAUDE.md, and RUNTIME_CONSTITUTION.md
~/.config/opencode/PAI/hooks/                     Hook scripts (39 files, .hook.ts + .hook.sh)
~/.config/opencode/PAI/hooks/handlers/            Handler modules (6 files)
~/.config/opencode/PAI/hooks/lib/                 Shared libraries (16 files)
~/.config/opencode/PAI/hooks/lib/learning-utils.ts Learning categorization
~/.config/opencode/PAI/hooks/lib/time.ts          PST timestamp utilities
~/.config/opencode/PAI/PAI/MEMORY/WORK/               Work tracking
~/.config/opencode/PAI/PAI/MEMORY/LEARNING/           Learning captures
~/.config/opencode/PAI/PAI/MEMORY/STATE/              Runtime state
~/.config/opencode/PAI/PAI/MEMORY/STATE/events.jsonl  Unified event log (append-only)
~/.config/opencode/PAI/PAI/MEMORY/OBSERVABILITY/      Tool failures, agent spawns, config changes

INFERENCE TOOL (for hooks needing AI):
Path: ~/.config/opencode/PAI/PAI/TOOLS/Inference.ts
Import: import { inference } from '../../.config/opencode/PAI/PAI/TOOLS/Inference'
...
```typescript
import { inference } from '../../.config/opencode/PAI/PAI/TOOLS/Inference';
...
// appendEvent() writes to ~/.config/opencode/PAI/PAI/MEMORY/STATE/events.jsonl
...
tail -f ~/.config/opencode/PAI/PAI/MEMORY/STATE/events.jsonl | jq

# Filter by type
tail -f ~/.config/opencode/PAI/PAI/MEMORY/STATE/events.jsonl | jq 'select(.type | startswith("algorithm."))'

# Programmatic (Node/Bun fs.watch)
import { watch } from 'fs';
const eventsPath = `${process.env.HOME}/.config/opencode/PAI/PAI/MEMORY/STATE/events.jsonl`;
