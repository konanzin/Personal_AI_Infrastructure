...
| `PAI/TOOLS/` helpers (ForgeProgress, AnvilProgress, CrossVendorAudit, Arthur engine) | Wrap external vendor CLIs and credential policy | Never published upstream; depend on external engine accounts (OpenAI for Forge/Cato; Anvil engine is per-machine config) | Agents return structured `unavailable`; helpers can be ported if multi-vendor work is adopted |
