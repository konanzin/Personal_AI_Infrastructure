...
- TTS is not initialized or called.
...
- Live STT, permission/question continuation, native shell-event rendering, reconnect/background behavior, and attachment picker/send still need direct validation.
...
- [x] ISC-5: Voice scope is STT-only and explicitly excludes TTS.
...
5. Validate the primary attachment picker/send UI or remove it from the active surface.
6. Add a small Tailscale URL helper in settings if the manual URL flow proves error-prone.
